    ...
    const appendDiv = () => {
        ...
        if (targetElement && targetElement.style.display !== 'none' && !targetElement.hasAttribute('data-appended')) {
            ...
            <div class="themesel" id="themesel">
              <button id="theme1">Custom Theme</button>
              <button id="theme2">Default Theme</button>
            </div>
...
// Use event delegation for the button event listeners
document.addEventListener('click', (event) => {
  const target = event.target;
  if (target.id === 'theme1' || target.id === 'theme2') {
